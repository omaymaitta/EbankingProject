...
public class EbankingBackendApplication {
	...
	return args -> {
		Stream.of("omayma", "adnane", "hassna").forEach(name -> {
		...
		bankAccountService.listCustomers().forEach(customer->{
			try {
				bankAccountService.saveCurrentBankAccount(Math.random()*9000, 9000L, customer.getId());
				bankAccountService.saveSavingBankAccount(Math.random()*120000, (long) 5.5, customer.getId());
...
CommandLineRunner start(CustomerRepository customerRepository,
	...
	return args -> {
		...
		Stream.of("omayma", "ikram", "hassna").forEach(name -> {
